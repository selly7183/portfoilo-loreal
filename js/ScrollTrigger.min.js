...
!(function (e, t) {
	...
	function Q(e, t) {
		...
		return (r = "scroll" + n) && (i = L(e, r))
			...
			: K(e)
			? Math.max(Pe[r], Oe[r]) -
			  (Me["inner" + n] || Pe["client" + n] || Oe["client" + n])
			: e[r] - e["offset" + n];
	}
	function R(e, t) {
	...
	function Da(e, t, r, n, o, i, a) {
		...
		return (
			(m += "position:" + (g && d ? "fixed;" : "absolute;")),
			(!g && d) ||
				(m += (n === rt ? b : y) + ":" + (i + parseFloat(f)) + "px;"),
			a &&
				(m +=
	...
	function Pa(e) {
		...
		if (g !== t || Fe) {
			for (A(); n < _.length; n += 3)
				Me.matchMedia(_[n]).matches
					? r.push(n)
					: A(1, _[n]) || (T(_[n + 2]) && _[n + 2]());
			for (E(), n = 0; n < r.length; n++)
				(Be = _[r[n]]), (_[r[n] + 2] = _[r[n] + 1](e));
		...
		if (i) {
			...
			(u = "scroll" + n.d2),
				w(
					...
					n,
					(b && 20 < m) ||
						(!b && (f ? Math.max(Oe[u], Pe[u]) : i.parentNode[u]) <= m + 1)
				),
				f &&
					((l = nt(a)),
	...
	var ke,
		...
		tt = {
			...
			sc: function sc(e) {
				return arguments.length
					? Me.scrollTo(e, rt.sc())
					: Me.pageXOffset || _e[t] || Pe[t] || Oe[t] || 0;
			},
		},
		rt = {
			s: r,
			...
			sc: function sc(e) {
				return arguments.length
					? Me.scrollTo(tt.sc(), e)
					: Me.pageYOffset || _e[r] || Pe[r] || Oe[r] || 0;
			},
		},
		...
		z = function _refreshAll(e, t) {
			if (!De || e) {
				var r = P("refreshInit");
				for (Ne && Y.sort(), t || A(), ze = 0; ze < at.length; ze++)
					at[ze].refresh();
				for (
		...
		B = function _updateAll() {
			...
			if (
				...
				(q = n),
				r &&
					(De && !Ae && 200 < t - De && ((De = 0), P("scrollEnd")),
					(Ie = v),
					(v = t)),
				lt < 0)
			) {
	...
	tt.op = rt;
	var Y =
		((ScrollTrigger.prototype.init = function init(b, y) {
			if (((this.progress = 0), this.vars && this.kill(1), Je)) {
				var d,
					...
					G,
					ee,
					te = (b = sa(S(b) || U(b) || b.nodeType ? { trigger: b } : b, it))
						.horizontal
						? tt
						: rt,
					re = b.onUpdate,
				...
				(Te.media = Be),
					...
					pe &&
						(V(pe) || (pe = { snapTo: pe }),
						ke.set(p ? [Oe, Pe] : he, { scrollBehavior: "auto" }),
						(l = T(pe.snapTo)
							? pe.snapTo
							: "labels" === pe.snapTo
					...
					se &&
						(!1 === le ||
							le === Ze ||
							(le = !(!le && "flex" === qa(se.parentNode).display) && $e),
						(Te.pin = se),
						!1 !== b.force3D && ke.set(se, { force3D: !0 }),
						(n = ke.core.getCache(se)).spacer
							? (z = n.pinState)
						...
						t !== w &&
							...
							se &&
								(t
									...
									: (de && Te.isActive) || ab(se, F, qa(se), j)),
							(w = t));
					}),
							...
							if (se && e && De) xa(ScrollTrigger, "scrollEnd", Qa);
								...
								for (
									var t,
										...
										g = b.end,
										h = b.endTrigger || ae,
										v = b.start || (se || !ae ? "0 0" : "0 100%"),
										x = (ae && Math.max(0, at.indexOf(Te))) || 0,
										m = x;
									m--;
								...
								for (
									_ =
										gb(v, ae, c, te, Te.scroll(), O, I, Te, f, ye, ve, u) ||
										(se ? -0.001 : 0),
										T(g) && (g = g(Te)),
										S(g) &&
											!g.indexOf("+=") &&
								...
								if (
									...
									(t = qa(se)),
										...
										(l = ve && M(he, o ? tt : rt)()),
										le &&
											(((j = [le + te.os2, A + d + et]).t = F),
											(m = le === $e ? ua(se, te) + A + d : 0) &&
												j.push(te.d, m + et),
											ct(j),
											ve && Te.scroll(G)),
										ve &&
									...
									for (r = ae.parentNode; r && r !== Oe; )
										r._pinOffset && ((_ -= r._pinOffset), (P -= r._pinOffset)),
											(r = r.parentNode);
								for (m = 0; m < x; m++)
									!(a = at[m].pin) ||
										(a !== ae && a !== se) ||
					...
					(Te.update = function (e, t) {
						...
						if (
							(t &&
								((k = C),
								(C = s),
								pe && ((X = m), (m = y && !ge ? y.totalProgress() : c))),
							fe &&
								!c &&
								se &&
								!Re &&
